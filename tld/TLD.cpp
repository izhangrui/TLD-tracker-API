...
BoundingBox bbnext;
bool lastboxfound = true;
void TLD::processFrame(const cv::Mat& img1, cv::Rect& box) {
	...
	if (lastboxfound && tl) {
		track(img2, img1, points1, points2);
	}
	else {
		tracked = false;
	}
	...
	box.x = bbnext.x;
	box.y = bbnext.y;
	box.width = bbnext.width;
	box.height = bbnext.height;
}
	...
	for (int s = 0; s<21; s++) {
		...
		for (int y = 1; y<img.rows - height; y += round(SHIFT*min_bb_side)) {
			for (int x = 1; x<img.cols - width; x += round(SHIFT*min_bb_side)) {
				...
				grid.push_back(bbox);
				/*if (bbox.overlap>0.6)
					cout << "bbox is:" << bbox.overlap << endl;*/
			}
		}
		sc++;
	}
...
float TLD::bbOverlap(const BoundingBox& box1, const BoundingBox& box2) {
	...
	//cout << "intersection / (area1 + area2 - intersection);" << intersection / (area1 + area2 - intersection) << endl;
...
void TLD::getOverlappingBoxes(const cv::Rect& box1, int num_closest) {
	...
	//cout << "good_booxes.size:" << num_closest << endl;直接编译运行参数加载会不成功
	...
	//cout << "good_booxes.size2:" << good_boxes.size() << endl;
	getBBHull();
}
...
void TLD::getBBHull() {
	...
	int y1 = INT_MAX, y2 = 0;
	int idx;
	// good_boxes.size = 0, mei you jingru xunhuan
